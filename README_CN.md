...
- 一个默认无声、可开启翻页音效的 **MP4 视频**，包含弯曲翻页、纸张边缘和阴影。
...
已安装 Node.js/npm 和 Git 时，运行：

```sh
npx skills add 109km/photos-to-album
```

如果出现选择提示，请选择 Codex 和所需的安装范围。CLI 只安装技能文件，**不会**运行本技能的渲染初始化。在安装后的文件夹中运行 `npm run setup`，或告诉 Codex：**“请为已安装的 photos-to-album 技能运行初始化，并验证渲染。”** 没有 Git 时，可使用下方的 ZIP 安装方式。
...
**如果安装已报告成功，无需再次运行初始化。** Codex 和手动安装说明均包含初始化；通过 Skills CLI 安装后，请按上文运行一次初始化。若你只是复制了技能文件夹，请在该文件夹中打开终端，执行：
...
翻页音效**默认关闭**。开启后，每次翻页开始时播放轻柔的纸张翻动声：
...
首次初始化会检查六种风格说明和附带的翻页录音，运行参数测试，并实际导出无声、有翻页音效的示例视频及书本图片。全部渲染素材都随仓库提供。AI 照片风格化仍需会话具备图片编辑工具；不同批次的生成画面可能不同。
